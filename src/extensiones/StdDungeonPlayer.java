	...
	*/
	...
	public void move() {
		...
		||   (xdir==0 && ydir==0 && (!isXAligned() || !isYAligned()) )  ) {
			...
			setDir(0,0);
			if (!stop_moving) {
				if (eng.getKey(key_left)&& (x- xdir)+ eng.tileWidth()>0 )  xdir = -1;
				if (eng.getKey(key_right)&& x + xdir<eng.pfWidth()) xdir = 1;
				if (eng.getKey(key_up) && y- ydir>0 )    ydir = -1;
				if (eng.getKey(key_down) && (y + ydir)+ eng.tileHeight()< eng.pfHeight())  ydir = 1;
			}
